...
- **Quick picks, waiting first** — a short grid of wide low tiles (`components/study/quick-picks.tsx`): All words, Sentences, then books, each showing its due count and a play affordance ONLY when there's something to drill (a play button that opens an empty session is a broken promise). Sorted by due count, so the row reorders itself through the day instead of being a fixed menu. Capped at six — past that it stops being shortcuts and becomes a worse copy of the library. This exact grid was cut from the Books page hours after it was built, where it sat above a shelf of the same links; on Home it isn't a duplicate, which is why every app with this row also has a separate library page
- **Shelves + recent chats** — your books as covers, then the official catalog (both via the shared `components/study/shelf.tsx`), then the last four chats that actually have messages in them (an empty draft thread is not something to "pick up where you left off" on)

...
- **Decks shelf (`/vocab/review`)** (2026-08-29) — the drill surface LANDS on a shelf, not a card: All words first (liked tile), then one row per book, each with cover art, `N due · M words`, a due pill and a play button. Arriving straight on a card meant the app picked the deck for you and never showed the others. `?book=all` is that old behaviour, now a choice; `?book=<id>` and `?pack=<slug>` unchanged. Official books ride the same page as a cover shelf (drillable without saving)
...
- **Mobile quick-access bar** (2026-08-29) — a fixed bottom tab bar on every study page (`components/shell/mobile-tabbar.tsx`): **Chat · Books · Decks · Official**, longest-path match so `/vocab/review` doesn't also light up Books. The drawer keeps the long tail (chats, projects, pinned books, reading list, account) — the bar exists to remove the open-then-choose tax on the four places people actually go. Its height is PUBLISHED as `--study-tabbar-h` (globals.css, `3.5rem + safe-area`, **0px at lg**) so the chat pane's height math and the floating Ask button size around it with one expression instead of hard-coding a number each; `study.spec` measures the rects rather than trusting the classes

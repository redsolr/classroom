...
| `npm run db:seed:paths` | Sync the learning-path catalog (`src/content/study-paths.ts`). Also runs on every deploy, right after the packs; fails loudly on a step pointing at a pack that doesn't exist **or asking for more words than that pack contains** (a node nobody can finish is worse than no node) |

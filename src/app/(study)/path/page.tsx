 ...
 * already decided to learn. This is the only page that has an opinion
 * about ORDER, which is the thing a beginner cannot supply for
 * themselves and the reason most self-study stalls in month two: not
 * lack of material, lack of a next thing.
 *
    ...
    <PageShell>
      ...
      <div className="max-w-3xl space-y-8">
        <section className="path-lead">
          <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <p className="mt-2 text-[0.8125rem] text-fg-tertiary">
                {lead.completedSteps} of {lead.steps.length} steps done
                {lead.enrolled ? " · following" : ""}
              </p>
            </div>

            {/* Following is a bookmark, not a commitment — it is what
                lets Home point at ONE path. So the button says what it
                does and nothing more dramatic. */}
            <form
              action={
                lead.enrolled
          ...
          <PathSteps steps={lead.steps} nextId={lead.next?.id} />
        </section>

        {others.length > 0 && (
          <section className="path-others">
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="block text-[0.8125rem] text-fg-tertiary">
                        {path.language} · {path.steps.length} steps
